...
\textbf{Niveau 1 -- Acquisition physique.} Ce niveau regroupe les capteurs et les actionneurs. Six capteurs de température sont interrogés via un bus de terrain industriel, et un débitmètre à effet Hall est lu par interruption matérielle. Un voyant lumineux assure le signalement visuel local des alertes.
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][6cm][c]{0.85\textwidth}{\centering\itshape [Figure 3.1 : Architecture globale du système à trois niveaux]}}
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][6cm][c]{0.85\textwidth}{\centering\itshape [Figure 3.2 : Architecture détaillée du système]}}
...
\begin{itemize}
  ...
  \item Un voyant lumineux adressable pour le signalement local.
\end{itemize}
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][6cm][c]{0.85\textwidth}{\centering\itshape [Figure 3.3 : Schéma matériel détaillé -- Raccordements électriques et bus de communication]}}
...
Le diagramme de la Figure~3.4 présente les acteurs et leurs interactions avec le système. Trois acteurs humains sont identifiés : l'opérateur de ligne, qui consulte la supervision et reçoit les alertes ; le responsable maintenance, qui accède au diagnostic et aux prédictions ; et l'administrateur, qui configure les seuils et déclenche les ré-entraînements. Un acteur système, le bus de terrain, interagit avec le système pour fournir les mesures brutes.
...
% actor "Bus terrain" as bus
% rectangle "Système de Supervision" {
%   usecase "Consulter températures" as UC1
%   usecase "Visualiser diagnostic" as UC2
%   usecase "Recevoir alerte" as UC3
%   usecase "Consulter prédictions" as UC4
%   usecase "Configurer seuils" as UC5
%   usecase "Déclencher ré-entraînement" as UC6
%   usecase "Acquérir mesures" as UC7
% }
% op --> UC1
% op --> UC2
% op --> UC3
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][8cm][c]{0.85\textwidth}{\centering\itshape [Figure 3.5 : Diagramme de classes du logiciel]}}
...
%   +send_alert()
% }
% Orchestrator o-- ModbusReader
% Orchestrator o-- FlowSensor
% Orchestrator o-- TimeSeriesDB
% Orchestrator o-- AlertManager
% Orchestrator --> GreyBoxModel
% Orchestrator --> AnomalyDetector
% Orchestrator --> CauseClassifier
% Orchestrator --> RidgePredictor
% MLModule <|.. GreyBoxModel
% MLModule <|.. AnomalyDetector
% MLModule <|.. CauseClassifier
% MLModule <|.. RidgePredictor
% @enduml
...
Le soft sensing est une technique qui consiste à estimer une grandeur physique difficile à mesurer directement à partir de mesures indirectes disponibles~[16]. Dans notre cas, l'épaisseur de calcaire accumulée dans les canalisations est inférée à partir de la température des moules et du débit d'eau, sans capteur dédié.
...
où \(\Delta T_{\text{normal}}\) représente les pertes thermiques inévitables du circuit (tuyauterie, raccords, échange) déterminées lors de la calibration initiale (tuyaux propres, jour~1), et \(\Delta T_{\text{calcaire}}\) est la perte additionnelle due à l'accumulation de calcaire.
...
La résistance thermique de la couche de calcaire est déduite de la loi de Fourier appliquée à une paroi cylindrique :
...
\begin{itemize}
  ...
  \item \textbf{Niveau~2 (N2) -- Modèle aléatoire (Random Forest)} : lorsque les règles physiques ne permettent pas de conclure (cas ambigus), le Random Forest est sollicité. Il reçoit un vecteur de dix caractéristiques et produit une distribution de probabilité sur les sept classes de défaillance. Si la confiance est suffisante, la cause est retenue ; sinon, la mention \texttt{CAUSE\_INDETERMINEE} est renvoyée.
\end{itemize}
...
Le modèle utilise la méthode de la forêt aléatoire (Random Forest), qui combine plusieurs arbres de décision entraînés sur des sous-échantillons de données. Chaque arbre produit une classe ; la classe majoritaire est retenue comme prédiction, et la proportion d'arbres ayant voté pour elle constitue le score de confiance.
...
La prédiction de maintenance consiste à estimer le nombre de jours restants avant que l'épaisseur de calcaire n'atteigne un seuil critique (1,5~mm), au-delà duquel une intervention de détartrage est impérative.

Le modèle utilisé est une régression Ridge, forme régularisée de la régression linéaire qui pénalise les grands coefficients par une norme~\(\ell_2\) pour éviter le sur-apprentissage~[19] :
...
\begin{itemize}
  ...
  \item \textbf{SYSTEM} (système) : défaillance du bus de terrain, perte de communication -- notification à l'administrateur.
\end{itemize}

Chaque alerte contient la cause identifiée, le niveau de confiance, la liste des moules affectés avec leur nomenclature, et les actions correctives préconisées par l'AMDEC.
